...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/01/13 15:08:18 by amoroziu          #+#    #+#             */
/*   Updated: 2019/01/15 16:18:36 by amoroziu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	add_first(t_asm *champ, t_token **cur)
{
				...
				((*cur)->type == DIRECT_LABEL) * 2))
	{
		...
		else
		{
			...
			add_to_code(champ, ft_strdup("\x00\x00\x00\x00"));
		}
	}
}
...
int			lld(t_asm *champ, t_token **cur)
{
	if (!valid(cur))
		return (0);
	add_to_code(champ, ft_strdup("\x0d"));
	add_codes(champ, cur);
	*cur = (*cur)->next;
	add_first(champ, cur);
	*cur = (*cur)->next;
